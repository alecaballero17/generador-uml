...
replies.push({actions:[{action:'createClass',name:'Rol',attributes:[{name:'nombre',type:'String'}]},{action:'addRelationship',source:'Usuario',target:'Rol',type:'association'},{action:'addOperation',name:'Usuario',operationName:'autenticar',returnType:'Boolean'}]});await run('agrega Rol, relaciona Usuario con Rol y agrega metodo autenticar');assert.equal(diagram().classes.length,2);assert.equal(diagram().relationships.length,1);assert.equal(diagram().classes[0].operations.length,1);assert.equal(diagram().classes[0].operations[0].name,'autenticar');
replies.push({actions:[{action:'deleteRelationship',source:'Usuario',target:'Rol'}]});await run('elimina relacion entre Usuario y Rol');assert.equal(diagram().relationships.length,0);
replies.push({actions:[{action:'deleteOperation',name:'Usuario',operationName:'autenticar'}]});await run('elimina metodo autenticar de Usuario');assert.equal(diagram().classes[0].operations.length,0);
replies.push({actions:[{action:'renameClass',oldName:'Rol',newName:'Perfil'}]});await run('renombra clase Rol a Perfil');assert.equal(diagram().classes[1].name,'Perfil');
