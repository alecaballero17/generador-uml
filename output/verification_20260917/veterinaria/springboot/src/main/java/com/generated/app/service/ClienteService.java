...
import com.generated.app.repository.ClienteRepository;
...
public class ClienteService {
    ...
    private Cliente entity(Long id) { return repository.findById(id).orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Registro inexistente")); }
    ...
    public ClienteDto toDto(Cliente entity) {
        ...
        dto.setMascotasIds(entity.getMascotas().stream().map(x -> x.getId()).toList());
        return dto;
    }
    private void apply(Cliente entity, ClienteDto dto) {
    ...
    public List<ClienteDto> findAll() { return repository.findAll().stream().map(this::toDto).toList(); }
    @Transactional(readOnly = true)
    public ClienteDto findById(Long id) { return toDto(entity(id)); }
    public ClienteDto create(ClienteDto dto) {
        if (dto.getId() != null) throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "El ID lo asigna el servidor");
        Cliente entity = new Cliente(); apply(entity, dto);
        return toDto(repository.saveAndFlush(entity));
    }
    public ClienteDto update(Long id, ClienteDto dto) {
        Cliente entity = entity(id);
